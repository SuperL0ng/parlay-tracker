(() => {
  'use strict';
  class AppController {
    constructor({storage,builder,sharing,viewer,dashboard}){
      this.storage=storage;this.builder=builder;this.sharing=sharing;this.viewer=viewer;this.dashboard=dashboard;this.started=false;this.routeVersion=0;this.$=id=>document.getElementById(id);this.onEvent=this.onEvent.bind(this);this.route=this.route.bind(this);this.onHashChange=this.onHashChange.bind(this);
      this.elementHandlers=[
        ...
        ['importTicketBtn','click',()=>{this.viewer.deactivate?.();this.dashboard.closeOverlays();this.closeCodeSheet();this.sharing.openImport()}],
    ...
    closeTransientUi({preserveShare=false,deactivateViewer=true}={}){this.dashboard.closeOverlays();this.closeCodeSheet();if(!preserveShare)this.sharing.close?.();if(deactivateViewer)this.viewer.deactivate?.()}
    ...
    openCodeSheet(){this.dashboard.closeOverlays();this.sharing.close?.();this.$('codeSheet').classList.remove('hide');this.$('codeSheetBackdrop').classList.remove('hide');document.body.classList.add('modalOpen')}
    closeCodeSheet(){this.$('codeSheet')?.classList.add('hide');this.$('codeSheetBackdrop')?.classList.add('hide');const share=this.$('ticketShareModal'),shareOpen=Boolean(share&&!share.classList.contains('hide')),filterOpen=Boolean(document.querySelector?.('.sortFilterPanel'));if(!shareOpen&&!filterOpen)document.body?.classList.remove('modalOpen')}
