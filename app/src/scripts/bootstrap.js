(() => {
  ...
  const requiredGlobals=['ParlayStorage','ParlayTrackerSources','ParlayTrackerEvaluator','ParlaySettlementService','TrackerService','BuilderController','SharingController','TicketViewController','DashboardController','BackupController','AppController'];
  ...
  async function create(){
    ...
    const instance=Object.freeze({storage,tracker,builder,sharing,viewer,dashboard,backup,app});window.parlayApp=instance;window.dispatchEvent(new CustomEvent('parlay:app-ready'));return instance;
  }
  ...
  async function stop(){if(state.promise&&!window.parlayApp)try{await state.promise}catch{}const current=window.parlayApp;if(!current){state.promise=null;return}for(const name of ['app','backup','dashboard','viewer','sharing','builder'])try{await Promise.resolve(current[name]?.stop?.())}catch{}try{delete window.parlayApp}catch{window.parlayApp=undefined}state.promise=null;window.dispatchEvent(new CustomEvent('parlay:app-stopped'))}
