(() => {
  ...
  const requiredGlobals=['ParlayStorage','ParlayTicketStateModel','ParlayTrackerSources','ParlayTrackerEvaluator','ParlaySettlementService','TrackerService','BuilderController','SharingController','TicketViewController','DashboardController','AppController'];
  ...
  async function create(){
    ...
    const storage=window.ParlayStorage,stateModel=window.ParlayTicketStateModel;
    const tracker=new window.TrackerService({storage,sources:window.ParlayTrackerSources,evaluator:window.ParlayTrackerEvaluator,settlement:window.ParlaySettlementService,stateModel});
    const builder=new window.BuilderController({storage});
    const sharing=new window.SharingController({storage});
    const viewer=new window.TicketViewController({storage,tracker,stateModel});
    const dashboard=new window.DashboardController({storage,tracker,stateModel,root:element('ticketList'),status:element('dashboardStatus')});
    const app=new window.AppController({storage,builder,sharing,viewer,dashboard}),started=[];
    try{
    ...
    const instance=Object.freeze({storage,stateModel,tracker,builder,sharing,viewer,dashboard,app});window.parlayApp=instance;window.dispatchEvent(new CustomEvent('parlay:app-ready'));return instance;
  }
  ...
  async function stop(){if(state.promise&&!window.parlayApp)try{await state.promise}catch{}const current=window.parlayApp;if(!current){state.promise=null;return}for(const name of ['app','dashboard','viewer','sharing','builder'])try{await Promise.resolve(current[name]?.stop?.())}catch{}try{delete window.parlayApp}catch{window.parlayApp=undefined}state.promise=null;window.dispatchEvent(new CustomEvent('parlay:app-stopped'))}
